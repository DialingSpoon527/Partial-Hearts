...
public abstract class ColorfulHeartRendererMixin {
    ...
    @WrapOperation(method = "renderPlayerHearts", at = @At(value = "INVOKE", target = "Lterrails/colorfulhearts/api/heart/drawing/Heart;draw(Lnet/minecraft/client/gui/GuiGraphicsExtractor;IIZZZ)V"))
    private void renderPartialHeart(Heart heartInstance, GuiGraphicsExtractor guiGraphics, int heartX, int heartY, boolean hardcore, boolean highlightContainer, boolean highlightHeart, Operation<Void> original, @Local(name = "player") Player player,
